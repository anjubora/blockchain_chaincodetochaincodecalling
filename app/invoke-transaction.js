...
const invokeChaincode = async function(peerNames, channelName, chaincodeName, fcn, args, username, org_name) {
	...
	try {
		...
		for (const i in proposalResponses) {
	       ...
	       // console.log("new history of transaction------------",tr[i]);
		...
		if (all_good) {
			logger.info(util.format('Successfully sent Proposal and received ProposalResponse: Status - %s, message - "%s", metadata - "%s", endorsement signature: %s',
				proposalResponses[0].response.status, proposalResponses[0].response.message,
				proposalResponses[0].response.payload, proposalResponses[0].endorsement.signature));
				console.log('new history of transaction------------')
				transaction_history=(proposalResponses[0].response.payload).toString('utf8');
